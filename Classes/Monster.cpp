...
void Monster::AIUpdate(float dt,const Vec2& posOfPlayer, Scene* scene)
{
	...
	/*****************************每隔0.2秒，怪物将刷新一次，检测是否需要射击**************************************/
	if ((fireTime > 0.2f&& fireTimeNum!=0)|| (fireTimeNum==0&&fireTime>2.f))
	{
		if(fabs(posOfMonster.y-posOfPlayer.y)<20)
...
void doubleAIcontrol(const Vec2& posOfPlayer, float dt, Scene* scene, Monster* ai1=nullptr, Monster* ai2=nullptr)
{
	/*
	*初始化时此时怪物1是主位，怪物2是将随机对玩家进行两面包夹芝士
	*/
	Monster* monsterFirst = ai1;
	Monster* monsterSecond = ai2;
	Vec2 posOfMonster1;
	Vec2 posOfMonster2;
	...
	if (monsterFirst != nullptr)
		posOfMonster1 = monsterFirst->sprite->getPosition();//获取此时怪物1位置

	if (monsterSecond != nullptr)
		posOfMonster2 = monsterSecond->sprite->getPosition();//获取此时怪物2位置
	...
	if (stepTime > 0.44)
	{
		...
		if (monsterFirst != nullptr && monsterSecond != nullptr)
		{
